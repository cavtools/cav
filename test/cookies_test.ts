...
test("Initial state of a bare request (no cookies)", {
  get: { foo: undefined },
  entries: [],
  has: { bar: false },
  isSigned: { baz: false },
...
test("Initial state of a request with 1 signed and 1 unsigned cookie", {
...
test("Switching cookies between signed and unsigned", {
  req: new Request("http://localhost/test", {
    headers: {
      "cookie": "a=WyJhIiwiYWJjMTIzIl0.X6o_6Iy1LpDyTx1RYcZAAm6vZMn0ohZD3pP3_ox4fxI; b=hello"
    },
  ...
  setCookies: [
    ["set-cookie", "a=def456"],
    ["set-cookie", "b=WyJiIiwid29ybGQiXQ.V7l9sq0M71XIcSBWK5hW6cnDKsJR5X5aqQvJKlT0nPw"],
  ],
});

test("Set cookies with custom path/domain that don't match request", {
  req: new Request("http://localhost/test", {
...
test("Delete cookies with custom path/domain that don't match request", {
  req: new Request("http://localhost/test", {
    headers: { cookie: "a=hello" },
  }),
  op: cookies => {
...
test("Set cookies with custom path/domain that match request", {
...
test("Delete cookies with custom path/domain that match request", {
  req: new Request("http://sub.localhost/test/path", {
    headers: { cookie: "a=hello; b=world" },
  }),
  op: cookies => {
...
test("Setting cookies to an expired date deletes them", {
  req: new Request("http://localhost/test", {
...
test("Setting a signed cookie with an expiration encodes that date", {
  req: new Request("http://localhost/test", {
    // Payload: ["wholetthe", "catsout"]
    headers: { cookie: "wholetthe=WyJ3aG9sZXR0aGUiLCJjYXRzb3V0Il0.WH7T_UfH3AoCqCU1j_QQGEEwtxILEaYoG9l3E5UhYy4" },
  }),
  keys: ["abc123"],
  op: cookies => {
  ...
  setCookies: [
    ...
    ["set-cookie", "wholetthe=WyJ3aG9sZXR0aGUiLCJkb2dzb3V0Iiw0ODExNjMwNDAwMDAwXQ.RTFIYQRp4_kfEuaVVDrguEL7MD-c54k7FOC2vWrIyxw; Expires=Tue, 23 Jun 2122 04:00:00 GMT"],
    // Payload: ["whowhowho", "who", 949381200000]
    ["set-cookie", "whowhowho=WyJ3aG93aG93aG8iLCJ3aG8iLDk0OTM4MTIwMDAwMF0.6LVSmFj-0_pknMCM-29oLx8XFCE-04-duykFq31WtY8; Expires=Tue, 01 Feb 2000 05:00:00 GMT"]
  ],
});

test("Reading a signed cookie with an expired date causes it to be deleted", {
  req: new Request("http://localhost/test", {
    // Payload: ["whowhowho", "who", 949381200000]
    headers: { cookie: "whowhowho=WyJ3aG93aG93aG8iLCJ3aG8iLDk0OTM4MTIwMDAwMF0.6LVSmFj-0_pknMCM-29oLx8XFCE-04-duykFq31WtY8" },
  }),
  keys: ["abc123"],
...
test("Signed cookie with the wrong format never gets decoded", {
  keys: "abc123",
  req: new Request("http://localhost/test", {
    headers: {
      // fail: a -> ["a"]
...
test("Signed cookies aren't decoded without the correct key", {
  // Correct key: abc123
  keys: "abc124",
  req: new Request("http://localhost/test", {
    // Payload: ["a", "world"]
    headers: { cookie: "a=WyJhIiwid29ybGQiXQ.PIlTQhOh56MTmcX1c6SFxRzdL3vy_SmOY8ZBSDf7ZrU" },
  }),
  get: {
...
test("Key rollover", {
  ...
  setCookies: [
    ...
    ["set-cookie", "b=WyJiIiwidGVzdCJd.wqOMI-CKGC2Igr40wbl0uAaxmY-zKHCc9QTWtaVrrOU"],
  ],
});

test("Signed cookies with a tampered name are never decoded", {
  keys: "abc123",
  req: new Request("http://localhost/test", {
    headers: { cookie: "b=WyJhIiwid29ybGQiXQ.PIlTQhOh56MTmcX1c6SFxRzdL3vy_SmOY8ZBSDf7ZrU" },
  }),
  get: {
...
test("Max-Age, Expires, and some other options", {
  keys: "abc123",
  req: new Request("http://localhost/test", {
    headers: { cookie: "a=hello; b=WyJiIiwid29ybGQiXQ.V7l9sq0M71XIcSBWK5hW6cnDKsJR5X5aqQvJKlT0nPw" },
  }),
  op: async cookies => {
    cookies.set("a", "world", {
      secure: true,
